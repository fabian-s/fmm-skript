---
name: deslop
description: Edit prose to remove LLM/AI-writing tells (inflated vocabulary, formulaic phrases, vague authority, chatbot scaffolding, mechanical triads, and formatting tics) without changing content, numbers, citations, or technical meaning. Use when asked to de-slop, humanize, remove AI tells, Claudismen, or mannered prose, or check a draft for chatbot phrasing, and proactively before finalizing a paper section, peer review, Gutachten, grant, teaching material, lecture script, tooltip text, or email. Works for English and German prose (German tells have their own list) and scales to multi-file documents with a grep-first, surgical-edit, commit-per-chapter procedure. For scientific manuscripts, also audit claim–evidence alignment, uncertainty, causal scope, reproducibility detail, and reporting completeness without inventing missing science. Preserve load-bearing domain terms such as robust, sandwich, leverage, penalized, coverage, efficient, significant, and power.
---
...
Companion references: `references/jargon-glossing.md` for a lighter, separate check
(undefined abbreviations); `references/german-tells.md` for German prose, where the
English word lists do not apply; `references/long-document-procedure.md` for anything
longer than one file or one sitting.
...
| **Lecture script / interactive textbook (MDX, tooltips)** | Teaching profile plus: one didactic lead-in per widget or example, not two; no lead-out that restates the widget ("Wie das Widget zeigt"); widget/remark/example *titles* are prose too and carry the tics in concentrated form; directive labels are plain text (no math, no markup); never rename anchors or `@ref` keys; keep the house dash budget; quiz explanations get the same pass, quiz intro lines ("Fünf Aussagen. Welche stimmen?") go. Metaphors for algorithms (Arbeitspferd, Notnagel, Canyon, kriecht/zappelt) are the dominant tell here — see `references/german-tells.md`. |
...
**Language.** For German, read `references/german-tells.md` before the pass; its
regex triages a whole document in one grep, and its metaphor table is the main edit
list. Register is wir-Form unless the document says otherwise.

**Scale.** For more than one file or ~20k words, follow
`references/long-document-procedure.md`: house style first, grep triage, read in
chunks, surgical string replacements with a uniqueness assertion (never whole-file
rewrites), project checks and a commit after every chapter, push early.
...
8. Titles and labels: rename mannered section/widget/example titles to the topic
   (teaching profile allows a question as title, not a joke or a drama colon); keep
   ids and anchors byte-identical.
9. For scientific prose, run the scientific-writing checklist — including its
...
- **Rewrite** — apply the fixes and return the edited text (or edit the file in place
  for an existing document: exact-string replacements, one batch per file, never a
  wholesale rewrite). Always show what changed, briefly.
...
- **Iterate** — after a rewrite, do one more detect pass over the result; if it's clean,
  ...
  approach isn't working, not that more scrubbing is needed. Exception: on a long
  document, keep a running list of the document's own pet phrases and re-grep the
  chapters already finished once at the end.
