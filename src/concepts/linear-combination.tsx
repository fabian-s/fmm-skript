...
registerConcept({
  ...
  body: (
    <>
      <p>
        Eine <em>Linearkombination</em> von{" "}
        <ConceptLink id="vector">Vektoren</ConceptLink>{" "}
        <M>{"\\bv_1, \\ldots, \\bv_k"}</M> entsteht, indem wir jeden Vektor mit
        einem <ConceptLink id="scalar">Skalar</ConceptLink> strecken und die
        Ergebnisse addieren:
      </p>
      <MD>{"c_1 \\bv_1 + c_2 \\bv_2 + \\cdots + c_k \\bv_k."}</MD>
      <p>
        Denken wir an ein Rezept: Die Vektoren sind die Zutaten, und die
        Skalare <M>{"c_1, \\ldots, c_k"}</M> geben an, wie viel von jeder wir
