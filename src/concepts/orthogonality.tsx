...
registerConcept({
  ...
  body: (
    <>
      <p>
        Zwei <ConceptLink id="vector">Vektoren</ConceptLink> heißen{" "}
        <em>orthogonal</em> (also: senkrecht), wenn sie sich im rechten
        Winkel treffen. Der algebraische Test benutzt das{" "}
        <ConceptLink id="dot-product">Skalarprodukt</ConceptLink>:
      </p>
      <MD>{"\\bx \\perp \\by \\quad \\Longleftrightarrow \\quad \\bx^T \\by = 0 ."}</MD>
      <p>
        Zum Beispiel sind <M>{"[1, 2]^T"}</M> und <M>{"[-2, 1]^T"}</M>{" "}
        orthogonal, denn <M>{"1 \\cdot (-2) + 2 \\cdot 1 = 0"}</M>. Rechte
        Winkel lassen Rechnungen mit der{" "}
        <ConceptLink id="euclidean-norm">2-Norm</ConceptLink> sauber
        zerfallen: Der Satz des Pythagoras{" "}
        <M>{"\\|\\bx + \\by\\|_2^2 = \\|\\bx\\|_2^2 + \\|\\by\\|_2^2"}</M>{" "}
        ...
        <ConceptLink id="span">Spann</ConceptLink> des Modells (vgl. Heath
