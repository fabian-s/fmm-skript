...
registerConcept({
  ...
  body: (
    <>
      <p>
        Eine Funktion <M>{"f"}</M> ist <em>stetig</em> (continuous) in einem
        Punkt <M>{"a"}</M>, wenn eine kleine Änderung der Eingabe nur eine
        kleine Änderung der Ausgabe bewirkt: nähert sich <M>{"x"}</M> dem
        Punkt <M>{"a"}</M>, so nähern sich die Werte <M>{"f(x)"}</M> dem Wert{" "}
