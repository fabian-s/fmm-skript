...
/**
 * §13.5 — DIE EINE EINSICHT: Im Optimum eines beschraenkten Problems sind
...
export function LagrangeGeometrie() {
  ...
  if (modus === "eq") {
    ...
    if (parallel) {
      status = `Zulässig ist die rote Halbebene x + y ≥ 1, das unbeschränkte Minimum (0; 0) liegt außerhalb. Hier stimmen beide Stationaritätsgleichungen überein und liefern μ = ${fmt(multAusX)} > 0, und wegen h(${fmt(x)}; ${fmt(y)}) = 0 ist auch die Komplementarität erfüllt. Alle vier Bedingungen von Satz 13.5.7 sind damit erfüllt, die Ungleichung ist aktiv, sie bindet: ∇f = (${fmt(gf[0])}; ${fmt(gf[1])}) und ${gradText} zeigen in entgegengesetzte Richtungen, und diese Gegenläufigkeit ist es, die μ positiv macht.`;
    } else {
    ...
    status = `Jetzt zeigt die Ungleichung in die andere Richtung, zulässig ist die rote Halbebene x + y ≤ 1. ${rand} Die duale Zulässigkeit aus Satz 13.5.7 verbietet das: Kein Randpunkt ist ein KKT-Punkt. Stattdessen gewinnt das unbeschränkte Minimum x* = (0; 0) mit f* = 0. Dort ist h(0; 0) = −1 < 0, die Ungleichung ist inaktiv, die Komplementarität μ·h = 0 erzwingt μ = 0, und die Stationarität wird trivial erfüllt, weil ∇f(0; 0) = (0; 0) ist.`;
  }
