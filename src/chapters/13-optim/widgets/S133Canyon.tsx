...
/**
 * §13.3 — DIE EINE EINSICHT: Die Kondition κ = L/μ erzeugt den Zickzack. Die
 ...
 * 13.3.15 ist dabei eine SCHRANKE, keine exakte Vorhersage (Beispiel 13.3.16).
 *
...
export function CanyonWidget() {
  ...
  if (Math.abs(faktor2) > 1 + 1e-12) {
    ...
    status = `γ = ${fmt(gamma, 4)} liegt über 2/L = ${fmt(2 / L, 4)}: In der steilen Richtung wächst der Fehler je Schritt um den Faktor ${fmt(Math.abs(faktor2), 2)}, die Folge läuft aus dem Bild. Satz 13.3.15 verlangt γ ≤ 1/L und sagt hier nichts mehr.`;
    ...
    status = `x₂ ist nach einem Schritt null. Danach fällt f in jedem Schritt auf das ${fmt(faktor1 ** 2, 4)}-fache, also SCHNELLER als die Schranke ρ = 1 − μ/L = ${fmt(rho, 4)} aus Satz 13.3.15. Genau das führt Beispiel 13.3.16 vor: Der Satz verspricht höchstens ρ pro Schritt, nicht genau ρ.`;
