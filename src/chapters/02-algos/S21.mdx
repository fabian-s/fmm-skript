...
Kapitel 3), und diese Fehler können sich *akkumulieren*. Der Fehler einer
...
- *exakt/direkt*: Es gilt $f(\bx) = \wt{f}(\bx)$ (bis auf
  Rundungsfehler). Beispiel: das
  :k[Gauß-Verfahren]{#gaussian-elimination} für
  lineare Gleichungssysteme: nach endlich vielen Schritten steht die exakte
  Lösung da, wäre da nicht die Gleitkommarundung.
- *approximativ*: Es gilt lediglich
  $f(\bx) \approx \wt{f}(\bx)$, und zwar schon in exakter Arithmetik.
  Beispiel: die nach endlich vielen Summanden abgebrochene Reihe für
  $e^x$.
- *iterativ*: Dieselbe Vorschrift wird mehrfach ausgeführt, und jede
  Wiederholung verbessert die aktuelle Näherung. Beispiel: das
  :k[Newton-Verfahren]{#newtons-method} oder das
  :k[Gradientenverfahren]{#gradient-descent}.
- *probabilistisch*: Das Verfahren beruht auf Zufall, es würfelt.
