...
assert.deepEqual(multiply(A, [3, 2]), [1, 1]);
assert.deepEqual(multiply(A, [1, 1]), [0, 0]);
assert.equal(w[0] * 1 + w[1] * 1, 0);
assert.equal(A[0][0] * A[1][1] - A[0][1] * A[1][0], 0);
console.log("KAP09 S93: A = v wᵀ, Kernprobe und Bildrichtung geprüft.");
